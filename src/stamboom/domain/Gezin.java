...
public class Gezin {
    ...
    boolean setHuwelijk(Calendar datum) {
        ...
        if (datum != null){
            if (huwelijksdatum == null){
                long agemilisec1 = new Date().getTime() - this.ouder1.getGebDat().getTime().getTime();
                long agemilisec2 = new Date().getTime() - this.ouder2.getGebDat().getTime().getTime();
                Date age1 = new Date(agemilisec1);
                Date age2 = new Date(agemilisec2);
                if (age1.getYear() >= 18 && age2.getYear() >= 18){
                    this.setHuwelijk(datum);
                    return true;
                }
            }
        }
    ...
    public String beschrijving() {
        ...
        String huwelijksdatstring = "";
        if (huwelijksdatum != null){
            huwelijksdatstring = huwelijksdatum.toString();
        }
        String beschrijving = "/(nr) /(ouder1.getAchternaam()) /(ouder2.getAchternaam()) /(huwelijksdatstring)";
